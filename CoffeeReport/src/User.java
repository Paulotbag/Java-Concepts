...
public class User {
	...
	/*
	 * This method checks that an int input is within a range, it depends on method
	 * inputInteger(String). If min is larger than max when passed, the method will
	 * swap the min and max so that min is always less than max.
	 */
	public static int inputInteger(String prompt, int min, int max) {
		...
		while (isInputBad) {
			...
			if (value >= min && value <= max) {
				...
				System.out.printf("Enter integer number between %d and %d (inclusive): ", min, max);
			}
		}
		return value;
	}
